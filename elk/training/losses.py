...
@register("ccs")
def ccs_squared_loss(logits: [Tensor], coef: float = 1.0) -> Tensor:
    ...
    """
    loss = consistency_squared_loss(logits) + confidence_squared_loss(
        logits
    )
    return coef * loss
...
def ccs_prompt_var_loss(logits: [Tensor], coef: float = 1.0) -> Tensor:
